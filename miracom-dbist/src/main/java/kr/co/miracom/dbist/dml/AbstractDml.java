...
import kr.co.miracom.dbist.annotation.GenerationRule;
...
import kr.co.miracom.dbist.processor.Preprocessor;
import kr.co.miracom.dbist.util.UuidGenerator;
import kr.co.miracom.dbist.util.ValueGenerator;
...
public abstract class AbstractDml implements Dml, ApplicationContextAware, BeanNameAware, InitializingBean {
    ...
    private static void applySelectOptions(Query query, SelectOptions options) {
        if (options == null) {
            return;
        }
        if (!ValueUtils.isEmpty(options.getSelect())) {
            query.addSelect(options.getSelect());
        } else if (!ValueUtils.isEmpty(options.getUnselect())) {
            query.addUnselect(options.getUnselect());
        }
    }

    @SuppressWarnings("unchecked")
    public <T> T select(T data, SelectOptions options) throws Exception {
        ValueUtils.assertNotNull("data", data);
        Class<T> clazz = (Class<T>) data.getClass();
        Query query = toPkQuery(clazz, data);
        applySelectOptions(query, options);
        return select(selectList(clazz, query));
    }

    @SuppressWarnings("unchecked")
    public <T> T selectWithLock(T data, SelectOptions options) throws Exception {
        ValueUtils.assertNotNull("data", data);
        Class<T> clazz = (Class<T>) data.getClass();
        Query query = toPkQuery(clazz, data);
        applySelectOptions(query, options);
    ...
    public <T> T select(Class<T> clazz, Object pkCondition, SelectOptions options) throws Exception {
    ...
    public <T> T selectWithLock(Class<T> clazz, Object pkCondition, SelectOptions options) throws Exception {
        ValueUtils.assertNotNull("clazz", clazz);
        ValueUtils.assertNotEmpty("pkCondition", pkCondition);
        Query query = toPkQuery(clazz, pkCondition);
        applySelectOptions(query, options);
    ...
    public <T> T select(String tableName, Object pkCondition, SelectOptions options, Class<T> requiredType)
                    throws Exception {
    ...
    public <T> T selectWithLock(String tableName, Object pkCondition, SelectOptions options, Class<T> requiredType)
                    throws Exception {
        ValueUtils.assertNotNull("tableName", tableName);
        ValueUtils.assertNotNull("pkCondition", pkCondition);
        ValueUtils.assertNotNull("requiredType", requiredType);
    ...
    public <T> Page<T> selectPage(Class<T> clazz, Query query) throws Exception {
        ...
        if (page.getIndex() >= 0 && page.getSize() > 0 && page.getTotalSize() > 0)
            page.setLastIndex((page.getTotalSize() / page.getSize())
                            - (page.getTotalSize() % page.getSize() == 0 ? 1 : 0));
    ...
    private String[] getFieldNames(Object condition) {
        ...
        return ValueUtils.isEmpty(query.getSelect()) ? new String[0]
                        : query.getSelect().toArray(new String[query.getSelect().size()]);
    }
    ...
    public <T> Page<T> selectPageByQl(String ql, Map<String, ?> paramMap, Class<T> requiredType, int pageIndex,
                    int pageSize) throws Exception {
    ...
    public <T> List<T> selectListBySql(String sql, Map<String, ?> paramMap, Class<T> requiredType, int pageIndex,
                    int pageSize) throws Exception {
    ...
    public <T> Page<T> selectPageBySql(String sql, Map<String, ?> paramMap, Class<T> requiredType, int pageIndex,
                    int pageSize) throws Exception {
    ...
    public <T> List<T> selectListBySqlPath(String sqlPath, Map<String, ?> paramMap, Class<T> requiredType,
    ...
    public <T> List<T> selectListBySqlPath(String sqlPath, Map<String, ?> paramMap, Class<T> requiredType,
        ...
        return selectListByQlPath(sqlPath, paramMap, requiredType, pageIndex, pageSize, firstResultIndex,
                        maxResultSize);
    }

    public <T> Page<T> selectPageBySqlPath(String sqlPath, Map<String, ?> paramMap, Class<T> requiredType,
                    int pageIndex, int pageSize) throws Exception {
    ...
    public <T> Page<T> selectPageBySqlPath(String sqlPath, Map<String, ?> paramMap, Class<T> requiredType,
                    int pageIndex, int pageSize, int firstResultIndex, int maxResultSize) throws Exception {
        return selectPageByQlPath(sqlPath, paramMap, requiredType, pageIndex, pageSize, firstResultIndex,
                        maxResultSize);
    }
    ...
    private <T> T _update(Class<T> clazz, Object data, String... fieldNames) throws Exception {
        ...
        T obj = select(clazz, data, null);
        if (obj == null) {
            Table table = getTable(data);
            throw new DataNotFoundException(
                            "Couldn't find data from table[" + table.getDomain() + "." + table.getName() + "]");
        }
    ...
    public <T> T upsert(Class<T> clazz, Object data) throws Exception {
        return select(clazz, data, null) == null ? insert(clazz, data) : update(clazz, data);
    }
    ...
    public void upsert(Class<?> clazz, Object data, String... fieldNames) throws Exception {
        if (select(clazz, data, null) == null)
    ...
    public <T> T delete(Class<T> clazz, Object pkCondition) throws Exception {
        T data = select(clazz, pkCondition, null);
        if (data == null)
            return null;
        delete(data);
        return data;
