 ...
 */
package kr.co.miracom.dbist.aspect;
 ...
 * 		&lt;/aop:aspect&gt;
 * 	&lt;/aop:config&gt;
 * 	&lt;bean id=&quot;sqlAspect&quot; class=&quot;kr.co.miracom.dbist.aspect.SqlAspect&quot;&gt;
...
public class SqlAspect {
    ...
    @Value("${miracom.sqlAspect.enabled}")
    private boolean enabled = true;
    @Value("${miracom.sqlAspect.prettyPrint}")
    private boolean prettyPrint;
    @Value("${miracom.sqlAspect.combinedPrint}")
    private boolean combinedPrint;
    @Value("${miracom.sqlAspect.includeElapsedTime}")
    ...
    private static final Comparator<String> COMPARATOR_REVERSED = ComparatorUtils
                    .reversedComparator(ComparatorUtils.naturalComparator());

    private void print(Object[] args) {
        if (!enabled || formatMethod == null || !logger.isInfoEnabled() || ValueUtils.isEmpty(args)
    ...
    private static String toParamValue(Object value) {
        ...
        if (value instanceof Date)
            return "'" + ValueUtils.toDateStr((Date) value, ValueUtils.DATEPATTERN_DATETIME) + "'";
