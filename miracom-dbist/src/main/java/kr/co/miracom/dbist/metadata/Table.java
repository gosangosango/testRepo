 ...
 */
package kr.co.miracom.dbist.metadata;
...
public class Table {
            ...
            synchronized (this) {
                ...
                for (Column column : getColumnList()) {
                    if (column.getRelation() != null
                                    || (column.getSequence() != null && column.getSequence().isAutoIncrement()))
                        continue;
    ...
    public String getUpdateSql(String... fieldNames) {
            ...
            synchronized (this) {
                ...
                for (Column column : getColumnList()) {
                    ...
                    if (column.isPrimaryKey()) {
                        ...
                        appendName(whereBuf, column.getName()).append(" = ").append(":")
        ...
        for (String fieldName : fieldNames) {
            ...
            if (column == null)
                throw new DbistRuntimeException("Invalid fieldName: " + getClazz().getName() + "." + fieldName);
            if (column.isPrimaryKey())
                throw new DbistRuntimeException("Updating primary key is not supported. " + getDomain() + "."
                                + getName() + getPkColumnNameList());
            buf.append(i++ == 0 ? "" : ", ");
            appendName(buf, toColumnName(fieldName)).append(" = :").append(fieldName);
        }
        StringBuffer whereBuf = new StringBuffer();
        for (String columnName : getPkColumnNameList())
            whereBuf.append(j++ == 0 ? " where " : " and ").append(columnName).append(" = ").append(":")
                            .append(toFieldName(columnName));
    ...
    public String getDeleteSql() {
        ...
        synchronized (this) {
            ...
            int i = 0;
            for (String columnName : getPkColumnNameList())
                buf.append(i++ == 0 ? " where " : " and ").append(columnName).append(" = :")
                                .append(getColumn(columnName).getField().getName());
